...
**Note:** Scripts generated by older plugin versions may lack `SHOW_EFFORT`. If the variable is not present in the script, OMIT the "Effort level" option from the wizard (toggling it would have no effect) and, after the update completes, mention that re-running `/statusline-wizard` will regenerate the script with effort level support.
...
- Effort level - select if SHOW_EFFORT=true
